...
Stage A combines two complementary ideas:
...
Stage B creates detector-level `*_uncal.asdf` files by iterating observation-plan rows and selected SCAs.
...
Stage C intentionally follows a canonical `romancal` progression:
...
Stage D exists to transform processed products into interpretable quality-control artifacts and science-facing summaries.
